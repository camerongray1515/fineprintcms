...
class Layouts extends FP_Controller {
	...
	function index()
	{
		...
		$layouts = $this->layout_model->get_all_layouts(FALSE);
	...
	function add()
	{
		...
		$editor = $this->editor_model->get_editor($editor_id, 'layout');
	...
	function set_default($layout_id = -1)
	{
		$this->load->model('layout_model');
		
		$success = TRUE;
		$message = "Layout has been set as default!";
		try
		{
			$this->layout_model->set_default($layout_id);
		}
		catch (exception $e)
		{
			$success = FALSE;
