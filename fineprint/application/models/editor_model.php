...
class Editor_model extends CI_Model {
	function get_editor($editor_id, $area = FALSE)
	{
		// If parameter supplied then we are adding a new item so need to allow the possibility of showing the default editor
		if ($area)
		{
			$editor_id = ($editor_id) ? $editor_id : $this->get_default_editor_id($area);
		}
	...
	function get_default_editor_id($area)
	{
		$this->load->model('setting_model');
		
		$editor_id = $this->setting_model->get("default_editor_$area");
